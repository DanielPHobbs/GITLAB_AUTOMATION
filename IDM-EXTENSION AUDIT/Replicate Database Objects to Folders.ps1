...
function global:DBObjectsToFolders ([string]$server, [string]$dbname){
    ...
    $SavePath = "G:\GITLAB_REPOS\gitlab_scorch_automation_IDMEXTENSIONrepo1"
    
    #$DateFolder = get-date -format yyyyMMddHHmm
    #new-item -type directory -name "$DateFolder"-path "$SavePath"
    
    foreach ($ScriptThis in $Objects | Where-Object {!($_.IsSystemObject)}) {
...
DBObjectsToFolders "dtekuk-mim.dtekuk.com" "IDM_EXTENSIONS" 